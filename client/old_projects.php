...
<body>
    ...
    <table id="old_projects_table" class="table table-striped table-bordered" style="width:100%">
        <thead>
            <tr>
                <th>Title</th>
                <th>Description</th>
...
async function getAllProjects() {
    ...
    if(contractObj) {                
        await contractObj.methods.totalProjects().call().then(function(totalProjects) {            
            if(totalProjects > 0) { 
                $("#old_projects_table").find("tr:gt(0)").remove();
                for(i=1;i<=totalProjects;i++) {                
                    contractObj.methods.getProject(i).call().then(function(project){ console.log(project);
                        if(project.projectStatus != 0) {
                            addProjectRow(project,'old_projects_table');
                        }    
                    });  
                }
            }        
